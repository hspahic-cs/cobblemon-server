...
- **Auction requests can now be for ANY item in the game**, not just a curated list. On the
  **Wanted** tab, **Create Request** opens a search box — type part of an item's name (e.g.
  `enchanted book`, `rare candy`, `netherite`) and pick from a grid of matching items, matched on
  display name *or* id, so anything you can find in JEI/EMI is requestable. Picking an item flows
  into the same quantity → price → escrow steps as before.
...
### Changed
- **Pack gym leaders / Elite Four now use the `pe` (poke-engine) AI**, matching our custom gyms.
  The 48 bdsp/radicalred/unbound leaders ship in the rctmod jar with the basic `rct` AI; new
  `server-pack-leader-ai` datapack re-emits each leader's full jar definition (team/rules intact)
  with `ai` swapped to `{"type": "pe", "data": {"temperature": 0.5}}`. Scoped to leaders only — the
  wild `hl_` filler stays on the lightweight in-process `rb` AI, so poke-engine load stays limited
  to boss fights. Generated by `ops/gen_pack_leader_ai.py`.
