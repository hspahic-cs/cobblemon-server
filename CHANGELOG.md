...
- **Battle Tower trainer titles now show the difficulty.** A floor's HARD and NORMAL tracks shared
  ...
  nameplate distinguished them. Both tracks' `name` now carries a mode marker — HARD in bold red
  with a skull (`§c§l☠ Penny [HARD]`) to emphasize the harder track, NORMAL plain (`Penny
  [Normal]`) — matching the nameplate convention in `TowerManager`. Stamped by
