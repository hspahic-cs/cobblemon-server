 ...
 * over it.
 *
 * ### Why a biome carries its arena template rather than sitting beside one
 *
 * §2.19 already re-stamps the arena when the wave band's template changes, and §2.24 says biomes
 * *are* those bands. Two lists — one saying which template wave 40 gets, another saying which biome
 * it gets — would be two answers to one question, and they would disagree the first time somebody
 * edited one of them: a volcano build under a snowy sky, with nothing in the log to say which half
 * was wrong. So the template id lives here, and [com.cobblemonroguelite.arena.ArenaConfig.templates]
 * stays as the answer for a server that has no biomes configured at all.
 *
 ...
 *   to put a key in.
 * @property arenaTemplate the build stamped while the run is here. Resolved by
 *   [com.cobblemonroguelite.arena.ArenaStamper], which is the layer that can say whether the `.nbt`
 *   exists; a template named here and missing on disk fails loudly there rather than quietly at load.
...
data class RunBiome(
    ...
    val arenaTemplate: ResourceLocation,
