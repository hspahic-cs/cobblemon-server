...
/**
 * The set of species a player has unlocked for the starter offer.
 *
...
fun interface CaughtSpeciesSource {
     ...
     *
     * Returns species ids only, with no indication of *how* each was earned. The offer builder
     * unions this with the baseline pool and the result is indistinguishable from that point on,
     * which is what stops unlock status from leaking into weighting (§2.15).
     */
 ...
 * a lookup for a UUID that never played creates a Pokédex file for it. That is fine for the offer,
 ...
 * an empty set here reduces the offer to the baseline pool, which §2.15 requires to be playable on
 * its own anyway.
 */
